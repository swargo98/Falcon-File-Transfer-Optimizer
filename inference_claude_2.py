...
class PPOAgentContinuous:
    ...
    def select_action(self, state):
        """Modified select_action method with better numerical stability"""
        with torch.no_grad():  # Add no_grad context for inference
            ...
            if torch.isnan(mean).any() or torch.isnan(std).any():
                print(f"Warning: NaN detected! Mean: {mean}, Std: {std}")
                # Fall back to last known good action or a default action
                return np.array([7.0, 10.0, 20.0]), np.zeros(3)  # These are empirically good values
                
            if self.inference_mode:
                # Use a very small but non-zero std dev for exploration
                reduced_std = torch.clamp(0.1 * std, min=1e-6, max=1.0)
                dist = Normal(mean, reduced_std)
                
                # Sample with added noise clipping
                action = dist.sample()
                action = torch.clamp(action, 1.0, 100.0)  # Clip to valid thread range
            else:
...
def inference_ppo(env, agent, num_steps=100):
    """Modified inference loop with better error handling"""
    ...
    try:
        for step in tqdm(range(num_steps), desc="Inference Steps"):
            # Add error handling around action selection
            try:
                action, action_logprob = agent.select_action(state)
            except Exception as e:
                print(f"Error in action selection: {e}")
                # Use last known good action or default
                action = np.array([7.0, 10.0, 20.0])
                action_logprob = np.zeros(3)
            ...
            if reward > best_reward:
                ...
                print(f"\nNew best state found at step {step}")
                print(f"Best reward: {best_reward:.2f}")
                print(f"Thread configuration: Read={action[0]:.1f}, Network={action[1]:.1f}, Write={action[2]:.1f}")
                print(f"Throughputs: Read={state[2]:.1f}, Network={state[4]:.1f}, Write={state[3]:.1f}")
            
            current_avg = np.mean(reward_window)
            
            # Update only if performance degrades significantly
            if current_avg < best_reward * 0.9:
                try:
                    agent.update(memory)
                except Exception as e:
            ...
            if (step + 1) % 10 == 0:
                ...
                print(f"\nStep {step + 1}")
                print(f"Average Reward (last 10 steps): {avg_reward:.2f}")
                print(f"Best Reward: {best_reward:.2f}")
...
if __name__ == '__main__':
    ...
    rewards = inference_ppo(env, agent)
