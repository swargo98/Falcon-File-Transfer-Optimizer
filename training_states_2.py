...
class SimulatorState:
    def __init__(self, sender_buffer_remaining_capacity=0, receiver_buffer_remaining_capacity=0,
                 read_throughput_change=0, write_throughput_change=0, network_throughput_change=0,
                 read_thread_change=0, write_thread_change=0, network_thread_change=0,
                 rewards_change=0) -> None:
...
def plot_throughputs_csv(throughputs_file='throughputs.csv', output_file='throughputs_plot.png'):
    ...
    # Compute rolling averages
    rolling_read = df['Read Throughput'].rolling(window=1).mean()
    rolling_network = df['Network Throughput'].rolling(window=1).mean()
    rolling_write = df['Write Throughput'].rolling(window=1).mean()
...
if __name__ == '__main__':
    ...
    agent = PPOAgentContinuous(state_dim=9, action_dim=3, lr=1e-4, eps_clip=0.1)
    ...
    policy_model = 'training_multiple_policy_20000.pth'
    value_model = 'training_multiple_value_20000.pth'
