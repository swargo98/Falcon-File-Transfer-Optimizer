...
class NetworkSystemSimulator:
    ...
    def get_utility_value(self, threads):
        ...
        if self.track_states:
            with open('threads_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', 'a') as f:
                f.write(f"{read_thread}, {network_thread}, {write_thread}\n")
            with open('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', 'a') as f:
...
def train_ppo(env, agent, max_episodes=1000, is_inference=False):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', 'a') as f:
        ...
        if episode % 1000 == 0:
            save_model(agent, "models/training_dicrete_w_history_minibatch_mlp_deepseek_v9_policy_"+ str(episode) +".pth", "models/training_dicrete_w_history_minibatch_mlp_deepseek_v9_value_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def plot_threads_csv(threads_file='threads_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', optimals = None, output_file='threads_plot.png'):
    optimal_read, optimal_network, optimal_write, _ = optimals
    data = []

    # Read data from threads_dicrete_w_history_minibatch_mlp_deepseek_v9.csv
    ...
    with open('average_threads_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', 'a') as f:
...
def plot_throughputs_csv(throughputs_file='throughputs_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', optimals = None, output_file='throughputs_plot.png'):
    optimal_throughput = optimals[-1]
    data = []

    # Read data from throughputs_dicrete_w_history_minibatch_mlp_deepseek_v9.csv
    ...
    with open('average_throughput_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', 'a') as f:
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v9_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v9_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v9.pdf')
    ...
    for i in range(inference_count):
        ...
        plot_rewards(rewards, 'PPO Inference Rewards', 'rewards/inference_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v9_'+ str(i) +'.pdf')
        plot_threads_csv('threads_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', optimals, 'threads/inference_threads_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v9_'+ str(i) +'.png')
        plot_throughputs_csv('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v9.csv', optimals, 'throughputs/inference_throughputs_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v9_'+ str(i) +'.png') 