...
class PolicyNetworkContinuous(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_size=256, activation=nn.ReLU):
        ...
        # Common feature extractor (backbone)
        ...
        for _ in range(action_dim):
            self.action_heads.append(
                nn.Sequential(
                    nn.Linear(hidden_size, hidden_size),
                    activation(),
                    nn.Linear(hidden_size, 2)  # Output mean and log_std for that action coordinate
                )
            )
    ...
    def forward(self, state):
        ...
        features = self.backbone(state)  # (batch_size, hidden_size)
        
        means = []
        stds = []
        for head in self.action_heads:
            head_out = head(features)  # (batch_size, 2)
            mean = head_out[:, 0]
            log_std = torch.clamp(head_out[:, 1], self.log_std_min, self.log_std_max)
            std = torch.exp(log_std)
            means.append(mean.unsqueeze(-1))
            stds.append(std.unsqueeze(-1))
        
        # Concatenate along the action dimension
        means = torch.cat(means, dim=-1)  # (batch_size, action_dim)
        stds = torch.cat(stds, dim=-1)    # (batch_size, action_dim)
        
        return means, stds

class ValueNetwork(nn.Module):
    def __init__(self, state_dim, hidden_size=256, activation=nn.ReLU):
...
class PPOAgentContinuous:
    ...
    def select_action(self, state):
        ...
        mean, std = self.policy_old(state)  # Now mean, std should be (1, action_dim)
        dist = Normal(mean, std)
        action = dist.sample()
        action_logprob = dist.log_prob(action)
        # Return as numpy (and optionally remove batch dimension if needed)
...
def train_ppo(env, agent, max_episodes=1000):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_coordinate.csv', 'a') as f:
        ...
        if episode % 10 == 0:
            save_model(agent, "models/coordinate_policy_"+ str(episode) +".pth", "models/coordinate_value_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'coordinate_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'coordinate_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_coordinate.pdf')
    ...
    plot_rewards(rewards, 'PPO Inference Rewards', 'inference_rewards_coordinate.pdf')