...
def train_ppo(env, agent, max_episodes=1000):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_residual.csv', 'a') as f:
        ...
        if episode % 1000 == 0:
            save_model(agent, "models/residual_policy_"+ str(episode) +".pth", "models/residual_value_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'residual_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'residual_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_residual.pdf')
    ...
    plot_rewards(rewards, 'PPO Inference Rewards', 'inference_rewards_residual.pdf')