...
class NetworkSystemSimulator:
    def __init__(self, read_thread = 1, network_thread = 1, write_thread = 1, sender_buffer_capacity = 10, 
                 ...
                 network_throughput_per_thread = 2, read_bandwidth = 6, write_bandwidth = 6, 
                 network_bandwidth = 6, read_background_traffic = 0, write_background_traffic = 0, 
                 network_background_traffic = 0, track_states = False, probing_interval = 3):
        ...
        self.optimal_read_thread = max(2, math.ceil(min_bandwidth // read_throughput_per_thread))
        self.optimal_network_thread = max(2, math.ceil(min_bandwidth // network_throughput_per_thread))
        self.optimal_write_thread = max(2, math.ceil(min_bandwidth // write_throughput_per_thread))
    ...
    def read_thread_task(self, time):
        ...
        if self.sender_buffer_in_use < self.sender_buffer_capacity:
            ...
            throughput_increase = min(read_throughput_temp, self.read_bandwidth * self.probing_interval -self.read_throughput)
        ...
        if next_time < self.probing_interval:
    ...
    def network_thread_task(self, time):
        ...
        if self.sender_buffer_in_use > 0 and self.receiver_buffer_in_use < self.receiver_buffer_capacity:
            ...
            throughput_increase = min(network_throughput_temp, self.network_bandwidth * self.probing_interval -self.network_throughput)
    ...
    def write_thread_task(self, time):
        ...
        if self.receiver_buffer_in_use > 0:
            ...
            throughput_increase = min(write_throughput_temp, self.write_bandwidth * self.probing_interval -self.write_throughput)
    ...
    def get_utility_value(self, threads):
        ...
        reward = (self.read_throughput/self.K ** read_thread) + (self.network_throughput/self.K ** network_thread) + (self.write_throughput/self.K ** write_thread)

        if self.track_states:
            with open('threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', 'a') as f:
                f.write(f"{read_thread}, {network_thread}, {write_thread}, {reward}\n")
            with open('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', 'a') as f:
                f.write(f"{self.read_throughput}, {self.network_throughput}, {self.write_throughput}, {reward}\n")
...
class SimulatorGenerator:
    def generate_simulator(self, episode=1):
        ...
        division_coefficient = divison_coefficients[min(3, int(env_cnt / 150))]

        sender_buffer_capacity = max(2, int(np.random.normal(loc=8, scale=1/factor))) * oneGB
        receiver_buffer_capacity = max(2, int(np.random.normal(loc=8, scale=1/factor))) * oneGB
        ...
        read_throughput_per_thread = max(30, int(np.random.normal(loc=333, scale=30/factor)))
        network_throughput_per_thread = max(30, int(np.random.normal(loc=333, scale=30/factor)))
        write_throughput_per_thread = max(30, int(np.random.normal(loc=333, scale=30/factor)))
...
def train_ppo(env, agent, max_episodes=1000, is_inference=False):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', 'a') as f:
        ...
        if episode % 1000 == 0:
            save_model(agent, "models/training_dicrete_w_history_minibatch_mlp_deepseek_v12_policy_"+ str(episode) +".pth", "models/training_dicrete_w_history_minibatch_mlp_deepseek_v12_value_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def plot_threads_csv(threads_file='threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', optimals = None, output_file='threads_plot.png'):
    optimal_read, optimal_network, optimal_write, _ = optimals
    data = []

    # Read data from threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv
    ...
    with open('average_threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', 'a') as f:
...
def plot_throughputs_csv(throughputs_file='throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', optimals = None, output_file='throughputs_plot.png'):
    optimal_throughput = optimals[-1]
    data = []

    # Read data from throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv
    ...
    with open('average_throughput_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', 'a') as f:
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v12_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v12_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    if os.path.exists('threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv'):
        os.remove('threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv')
    if os.path.exists('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv'):
        os.remove('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv')
    ...
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v12.pdf')
    ...
    for i in range(inference_count):
        ...
        policy_model = 'training_dicrete_w_history_minibatch_mlp_deepseek_v12_policy_400000.pth'
        value_model = 'training_dicrete_w_history_minibatch_mlp_deepseek_v12_value_400000.pth'
        ...
        plot_rewards(rewards, 'PPO Inference Rewards', 'rewards/inference_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v12_'+ str(i) +'.pdf')
        plot_threads_csv('threads_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', optimals, 'threads/inference_threads_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v12_'+ str(i) +'.png')
        plot_throughputs_csv('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v12.csv', optimals, 'throughputs/inference_throughputs_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v12_'+ str(i) +'.png') 