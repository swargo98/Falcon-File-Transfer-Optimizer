...
class NetworkSystemSimulator:
    def __init__(self, read_thread = 1, network_thread = 1, write_thread = 1, sender_buffer_capacity = 10, receiver_buffer_capacity = 10, read_throughput_per_thread = 3, write_throughput_per_thread = 1, network_throughput_per_thread = 2, read_bandwidth = 6, write_bandwidth = 6, network_bandwidth = 6, read_background_traffic = 0, write_background_traffic = 0, network_background_traffic = 0, track_states = False):
        ...
        self.optimal_reward = ((min_bandwidth/self.K ** optimal_read_thread) 
                               + (min_bandwidth/self.K ** optimal_network_thread) 
                               + (min_bandwidth/self.K ** optimal_write_thread))
    ...
    def get_utility_value(self, threads):
        ...
        reward = ((utility/self.optimal_reward)) * 1000
        ...
        final_state = SimulatorState(self.sender_buffer_capacity-self.sender_buffer_in_use,
                                    ...
                                    self.read_throughput - self.prev_read_throughput,
                                    self.write_throughput - self.prev_write_throughput,
                                    self.network_throughput - self.prev_network_throughput,
                                    read_thread - self.read_thread,
                                    write_thread - self.write_thread,
                                    network_thread - self.network_thread,
                                    read_thread,
                                    write_thread,
                                    network_thread,
                                    -1 if reward-self.reward < 0 else 1
                                    )                                    
...
class SimulatorGenerator:
    def generate_simulator(self):
        oneGB = 1024
        sender_buffer_capacity = np.random.randint(4, 10) * oneGB
        receiver_buffer_capacity = np.random.randint(4, 10) * oneGB
        read_throughput_per_thread = np.random.randint(50, 300)
        network_throughput_per_thread = np.random.randint(50, 300)
        write_throughput_per_thread = np.random.randint(50, 300)
        read_bandwidth = np.random.randint(2, 15) * oneGB
        write_bandwidth = np.random.randint(2, 15) * oneGB
        network_bandwidth = np.random.randint(2, 15) * oneGB
...
def train_ppo(env, agent, max_episodes=1000):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_training_multiple_new_reward.csv', 'a') as f:
        ...
        if episode % 1000 == 0:
            save_model(agent, "models/training_multiple_new_reward_policy_1000_"+ str(episode) +".pth", "models/training_multiple_new_reward_value_1000_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def plot_threads_csv(threads_file='threads.csv', optimals = None, output_file='threads_plot.png'):
...
def plot_throughputs_csv(throughputs_file='throughputs.csv', optimals = None, output_file='throughputs_plot.png'):
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'training_multiple_new_reward_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'training_multiple_new_reward_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    rewards = train_ppo(env, agent, max_episodes=80000)
    
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_training_multiple_new_reward_200k_1000.pdf')
    plot_threads_csv('threads.csv', 'training_threads_plot_training_multiple_new_reward.png')
    plot_throughputs_csv('throughputs.csv', 'training_throughputs_plot_training_multiple_new_reward.png')
    ...
    policy_model = 'training_multiple_new_reward_policy_200000.pth'
    value_model = 'training_multiple_new_reward_value_200000.pth'
    ...
    plot_rewards(rewards, 'PPO Inference Rewards', 'inference_rewards_training_multiple_new_reward.pdf')
    plot_threads_csv('threads.csv', 'inference_threads_plot_training_multiple_new_reward.png')
    plot_throughputs_csv('throughputs.csv', 'inference_throughputs_plot_training_multiple_new_reward.png')
    ...
    for i in range(inference_count):
        ...
        policy_model = 'training_multiple_new_reward_policy_100_200000.pth'
        value_model = 'training_multiple_new_reward_value_100_200000.pth'
        ...
        plot_rewards(rewards, 'PPO Inference Rewards', 'rewards/inference_rewards_training_multiple_new_reward_claude_'+ str(i) +'.pdf')
        plot_threads_csv('threads.csv', optimals, 'threads/inference_threads_plot_training_multiple_new_reward_claude_'+ str(i) +'.png')
        plot_throughputs_csv('throughputs.csv', optimals, 'throughputs/inference_throughputs_plot_training_multiple_new_reward_claude_'+ str(i) +'.png') 