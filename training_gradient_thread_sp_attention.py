...
class NetworkSystemSimulator:
    ...
    def get_utility_value(self, threads):
        ...
        if self.track_states:
            with open('threads_training_gradient_thread_sp_attention.csv', 'a') as f:
                f.write(f"{read_thread}, {network_thread}, {write_thread}\n")
            with open('throughputs_training_gradient_thread_sp_attention.csv', 'a') as f:
...
def train_ppo(env, agent, max_episodes=1000, is_inference=False, std_scale=0.1):
    ...
    for episode in tqdm(range(1, max_episodes + 1), desc="Episodes"):
        ...
        with open('episode_rewards_training_gradient_thread_sp_attention.csv', 'a') as f:
        ...
        if episode % 1000 == 0:
            save_model(agent, "models/training_gradient_thread_sp_attention_policy_"+ str(episode) +".pth", "models/training_gradient_thread_sp_attention_value_"+ str(episode) +".pth")
            print("Model saved successfully.")
...
def plot_threads_csv(threads_file='threads_training_gradient_thread_sp_attention.csv', optimals = None, output_file='threads_plot.png'):
    optimal_read, optimal_network, optimal_write, _ = optimals
    data = []

    # Read data from threads_training_gradient_thread_sp_attention.csv
...
def plot_throughputs_csv(throughputs_file='throughputs_training_gradient_thread_sp_attention.csv', optimals = None, output_file='throughputs_plot.png'):
    optimal_throughput = optimals[-1]
    data = []

    # Read data from throughputs_training_gradient_thread_sp_attention.csv
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'training_gradient_thread_sp_attention_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'training_gradient_thread_sp_attention_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

if __name__ == '__main__':
    # if os.path.exists('threads_training_gradient_thread_sp_attention.csv'):
    #     os.remove('threads_training_gradient_thread_sp_attention.csv')
    # if os.path.exists('throughputs_training_gradient_thread_sp_attention.csv'):
    #     os.remove('throughputs_training_gradient_thread_sp_attention.csv')
    ...
    # plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_training_gradient_thread_sp_attention.pdf')
    ...
    for i in range(inference_count):
        ...
        policy_model = 'training_gradient_thread_sp_attention_policy_250000.pth'
        value_model = 'training_gradient_thread_sp_attention_value_250000.pth'
        ...
        plot_rewards(rewards, 'PPO Inference Rewards', 'rewards/inference_rewards_training_gradient_thread_sp_attention_'+ str(i) +'.pdf')
        plot_threads_csv('threads_training_gradient_thread_sp_attention.csv', optimals, 'threads/inference_threads_plot_training_gradient_thread_sp_attention_'+ str(i) +'.png')
        plot_throughputs_csv('throughputs_training_gradient_thread_sp_attention.csv', optimals, 'throughputs/inference_throughputs_plot_training_gradient_thread_sp_attention_'+ str(i) +'.png') 