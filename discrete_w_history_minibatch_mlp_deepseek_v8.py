...
class NetworkSystemSimulator:
    ...
    def get_utility_value(self, threads):
        ...
        if self.track_states:
            with open('threads_dicrete_w_history_minibatch_mlp_deepseek_v8.csv', 'a') as f:
                f.write(f"{read_thread}, {network_thread}, {write_thread}\n")
            with open('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v8.csv', 'a') as f:
...
def find_last_policy_model():
    ...
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v8_policy_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]

def find_last_value_model():
    models = os.listdir("models")
    models = [model for model in models if re.match(r'training_dicrete_w_history_minibatch_mlp_deepseek_v8_value_\d+\.pth', model)]
    models.sort(key=lambda x: int(re.search(r'\d+', x).group()))
    return models[-1]
...
if __name__ == '__main__':
    ...
    plot_rewards(rewards, 'PPO Training Rewards', 'training_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v8.pdf')
    ...
    for i in range(inference_count):
        ...
        plot_rewards(rewards, 'PPO Inference Rewards', 'rewards/inference_rewards_training_dicrete_w_history_minibatch_mlp_deepseek_v8_'+ str(i) +'.pdf')
        plot_threads_csv('threads_dicrete_w_history_minibatch_mlp_deepseek_v8.csv', optimals, 'threads/inference_threads_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v8_'+ str(i) +'.png')
        plot_throughputs_csv('throughputs_dicrete_w_history_minibatch_mlp_deepseek_v8.csv', optimals, 'throughputs/inference_throughputs_plot_training_dicrete_w_history_minibatch_mlp_deepseek_v8_'+ str(i) +'.png') 